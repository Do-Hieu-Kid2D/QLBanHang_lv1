...
/// Cả bài này thầy chỉ cần LIB này để tương tác với Database thôi! >< 
namespace GUI
{
    public class SqlServer
    {
        ...
        public SqlCommand GetCmdSQLChay(string querySQL)
        {
            SqlCommand cmd = new SqlCommand(querySQL);
            cmd.CommandType = CommandType.Text;
            return cmd;
        }
        public int QueryNon(SqlCommand cm)
        {
            using (SqlConnection cn = new SqlConnection(ConnectionString))
            {
                ...
                cn.Open();
                cm.Connection = cn;
                int kq = cm.ExecuteNonQuery();//thực thi SQL -> trả về dr
                return kq;  //trả về kq
                //}catch (Exception ex)
                //{
                //    return ex.Message;
